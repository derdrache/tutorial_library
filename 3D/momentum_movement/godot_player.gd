...
func _physics_process(delta: float) -> void:
	...
	if direction:
		speed = move_toward(speed, abs(input_dir.y) * maxSpeed, acceleration * delta)
		
		velocity.x = direction.x * speed
		velocity.z = direction.z * speed
	else:
		speed = move_toward(speed, 0, slowDown * delta)
		
		velocity.x = move_toward(velocity.x, 0, slowDown * delta)
		velocity.z = move_toward(velocity.z, 0, slowDown * delta)
...
func _set_animation(direction):
	if direction:
		...
		animation_player.speed_scale = speed / maxSpeed * 2
