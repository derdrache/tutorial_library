...
func _input(event):
	...
	if Input.is_action_pressed("shift") and not usedRail:
		_start_rail()
	elif Input.is_action_just_pressed("ui_accept") and usedRail:
		_stop_rail()
...
func _start_rail():
	var onRail = shape_cast_3d.is_colliding() and shape_cast_3d.get_collider(0) is Rail
	
	if onRail:
		usedRail = shape_cast_3d.get_collider(0)
		usedRail.start_slide()
