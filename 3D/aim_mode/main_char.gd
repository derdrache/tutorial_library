...
@onready var character_duck_2: Node3D = $rotationNode/character_duck2
...
func _activate_aim_mode():
	aimMode = true
	third_person_camera.set_aim_mode(true)
	character_duck_2.hide()

func _deactivate_aim_mode():
	aimMode = false
	third_person_camera.set_aim_mode(false)
	character_duck_2.show()
...
func _physics_process(delta: float) -> void:
	...
	if not global_position.is_equal_approx(global_position + direction):
		rotation_node.look_at(global_position + direction, Vector3.UP, true)
	
	if aimMode:
		var cameraDirection = -third_person_camera.global_basis.z
		cameraDirection.y = 0
		rotation_node.look_at(global_position + cameraDirection, Vector3.UP, true)
