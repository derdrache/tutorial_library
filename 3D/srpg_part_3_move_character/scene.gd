...
func _input(event: InputEvent) -> void:
	if Input.is_action_just_pressed("left_mouse_click"):
		...
		if isPlayerCharacter:
			currentCharacter = collisionObject
			grid.remove_highlight()
			_highlight_movement_options(collisionObject)
		elif currentCharacter and collisionObject.isHighlighted:
			var moveSequence = grid.get_move_sequence(currentCharacter.global_position, collisionObject.global_position)
			currentCharacter.move(moveSequence)
			grid.remove_highlight()
